...
(defn messages-initial-state
  "Step 2a: Modify this dictionary so that there are at least two channels, which
   ...
   Step 2b: Modify the channels-list function and replace the static list that
   you created with a call to fetch the keys from the msgs dictionary parameter.
   If you need help, try googling 'clojure keys' (skip the article on
   destructuring).
   ...
   "
  []
  {"default" [{:msg "Modify me!" :time 0 :user {:name "Your Name" :nickname "You"}}]})

(defn messages-get
  "Step 3: Update this function so that it looks up the messages stored in
   the 'msgs' dictionary under the 'channel' key and returns them.

   If you complete this step successfully, you should be able to see the
   messages in the list of chat messages that appear when the page is loaded.
  "
  [msgs channel]
  (get (messages-initial-state) "default"))
...
(defn messages-add
  "Step 4: Update the messages add function so that it appends the
   msg passed to the function to the chnl-msgs variable declared in
   the let statement.
